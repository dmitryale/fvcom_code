function [finfo outstrct]=read_nc4_file_struct(fnam);
...
%   not want this feature comment line 36 #38.
%
%NOTES:
...
% nargoutchk(minargs,maxargs)   ;  %check if more than just finfo is declared %
% but make sure that the number of variables declared is not more than what is in the file



for i=1:length(finfo.Variables) ,...        %loop over the number of variables in the file
  try   eval(['a=ncread(fnam,''' finfo.Variables(i).Name ''');']) ;   %reads in variable data
